...
class User extends Common_API_Controller {
    ...
    function login_md_steward_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($isLogin)) {
                ...
                $return['status'] = 0;
                $return['message'] = DEACTIVATE_USER;
            } else if ($isLogin->delete_status == 1) {
                $return['status'] = 0;
                $return['message'] = DEACTIVATE_USER;
            } else {
    ...
    function careUnit_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $Roles = $this->common_model->customGet(array('table' => 'users_groups', 'select' => 'id', 'where' => array('user_id' => $this->user_details->id,'group_id' => 4)));
            if(!empty($Roles)){
                $option = array('table' => 'care_unit CU', 
                    'select' => 'CU.id,CU.name,CU.care_unit_code',
                    'join' => array('users as UG' => 'UG.care_unit_id=CU.id'),
                    'where' => array('CU.is_active' => 1, 'CU.delete_status' => 0,'UG.id'=>$this->user_details->id)
                );   
            }else{
                $option = array('table' => 'care_unit CU', 
                    'select' => 'CU.id,CU.name,CU.care_unit_code',
                    'where' => array('CU.is_active' => 1, 'CU.delete_status' => 0)
                );
            }
    ...
    function culture_source_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $states = $this->common_model->customGet(array('table' => 'culture_source', 'select' => 'id,name', 'where' => array('is_active' => 1, 'delete_status' => 0)));
            if ($states) {
                $return['status'] = 1;
                $return['response'] = $states;
                $return['message'] = 'Successfully records found';
    ...
    function organism_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $states = $this->common_model->customGet(array('table' => 'organism', 'select' => 'id,name', 'where' => array('is_active' => 1, 'delete_status' => 0)));
            if ($states) {
                $return['status'] = 1;
                $return['response'] = $states;
                $return['message'] = 'Successfully records found';
    ...
    function precautions_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $states = $this->common_model->customGet(array('table' => 'precautions', 'select' => 'id,name', 'where' => array('is_active' => 1, 'delete_status' => 0)));
            if ($states) {
                $return['status'] = 1;
                $return['response'] = $states;
                $return['message'] = 'Successfully records found';
    ...
    function add_patient_post() {
       ...
       // $this->form_validation->set_rules('md_stayward_consult', 'MD Stayward Consult', 'trim|required|in_list[Yes,No]');
        ...
        $this->form_validation->set_rules('initial_dot', 'Initial Days of Therapy', 'trim|required|numeric');
        ...
        $this->form_validation->set_rules('new_initial_dot', 'New Initial Days of Therapy', 'trim');
        ...
        $this->form_validation->set_rules('total_day_patient_stay', 'Total Days of Patient Stay', 'trim');
        $this->form_validation->set_rules('patient_unique_id', 'Patient Id', 'trim|required|min_length[1]|max_length[12]');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $patient_unique_id = strtoupper($CareUnit->care_unit_code).''.$this->input->post('patient_unique_id');
            $option = array(
                'table' => 'patient',
                'where' => array(
            ...
            /* if (!empty($IsPatientUniqueID)) {
             ...
             $x =   $this->input->post('precautions');
            if($x == null || $x == 'null' || $x == 'Please Select Precautions' || $x == '' || $x == 'Please Select Precautions '){
                $return['status'] = 0;
                $return['message'] = 'The Precautions field is required.';
                $this->response($return);
                exit;
            }
            ...
            if ($patient_id) {
                ...
                $return['response']['patient_id_show'] = $patient_unique_id;
     ...
     * Description:   To Get All category
     */
    // function add_patient_existing_post() {
    //     $data = $this->input->post();
    //     $return['code'] = 200;
    //     $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
    //     //$this->form_validation->set_rules('name', 'Name', 'trim');
    ...
    //     $this->form_validation->set_rules('symptom_onset', 'Infection Onset', 'trim|required|in_list[Hospital,Facility]');
    //     // $this->form_validation->set_rules('culture_source', 'Culture Source', 'trim|required|in_list[NA,Sputum,Stool,Wound,Urine,Blood,Nares]');
    //     // $this->form_validation->set_rules('organism', 'organism', 'trim|required|in_list[NA,Candida,C. auris,Citrobacter,Cdiff,Coag Neg Staph,COVID-19,Enterobacter,Enterococcus,Ecoli,ESBL ecoli,ESBL klebsiella,Klebsiella,MDRO,MRSA,MSSA,Proteus,Pseudomonas,Streptococcus,VRE,Other,COVID-19]');
    //     $this->form_validation->set_rules('culture_source', 'Culture Source', 'trim|required');
    //     $this->form_validation->set_rules('organism', 'Organism', 'trim|required');
    //     $this->form_validation->set_rules('precautions', 'Precautions', 'trim|required');
    //     $this->form_validation->set_rules('care_unit_id', 'Care Unit', 'trim|required|numeric');
    ...
    function notification_list_post() {
       ...
       // $this->form_validation->set_rules('NO');
        if ($this->form_validation->run() == FALSE) {
            $error = $this->form_validation->rest_first_error_string();
            $return['status'] = 0;
            $return['message'] = $error;
            ...
            $option = array(
                'table' => 'notifications',
                'select' => 'id, type_id as patient_id,message,read_status,sent_time,care_unit_id ',
                'order' => array('id' => 'DESC' )
            );
            $option['where']['user_id'] = $data['md_steward_id'];
           // $option['where']['read_status'] = "NO";
           $option['where']['is_removed'] = 0;
    ...
    function notification_list_delete_post() {
        $data = $this->input->post();
        $return['code'] = 200;
             $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
            $this->form_validation->set_rules('md_steward_id', 'MD Steward Id', 'trim|required|numeric');
            ...
            $option = array(
                    ...
                    'where' => array('id' => $data['notification_id'],'user_id' => $data['md_steward_id'])
            );
            
            if ($data['user_role'] === 'Md Steward') {
            $this->common_model->customupdate($option);
            }
              //  $option['where']['read_status'] = "YES";
        $notification_listss = $this->common_model->customGet($option);
        
        if ($notification_listss) {
                $return['response'] = $notification_listss;
            $return['status'] = 1;
            $return['message'] = 'Notification removed successfully';
        } else {
            $return['status'] = 0;
            $return['message'] = 'Records not updated';
        }
        }
        $this->response($return);
    }
        ...
        function notificationStatusUpdate_list_post() {
            $data = $this->input->post();
            $return['code'] = 200;
            $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
            $this->form_validation->set_rules('md_steward_id', 'MD Steward Id', 'trim|required|numeric');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $option = array(
                        ...
                        'where' => array('type_id' => $data['type_id'],'user_id' => $data['md_steward_id'])
                );
                $this->common_model->customupdate($option);

                 $option['where']['read_status'] = "YES";
            $notification_lists = $this->common_model->customGet($option);
    ...
    function patient_list_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array(
                'table' => 'patient P',
                'select' => 'P.id as pid, P.patient_id as patient_id,P.md_patient_status,P.name as patient_name,P.date_of_start_abx,P.address,P.symptom_onset,P.md_stayward_consult,P.criteria_met,P.md_stayward_response,P.created_date,'
                . 'P.care_unit_id,CI.name as care_unit_name,P.doctor_id,DOC.name as doctor_name,P.culture_source,P.organism,P.precautions,CS.name as culture_source_name,Org.name as organism_name,Pre.name as precautions_name,P.md_steward_id,U.first_name as md_steward,'
            ...
            if (!empty($data['patient_id'])) {
                $option['where']['P.id'] = $data['patient_id'];
            }
     ...
     * Description:   To Get All category
     */
    function patient_list_existing_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
    ...
    function patient_details_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $option = array(
                'table' => 'patient P',
                'select' => 'P.id as pid,P.md_patient_status,P.infection_surveillance_checklist,P.total_days_of_patient_stay,P.date_of_start_abx,P.patient_id as patient_id,P.name as patient_name,P.address,P.symptom_onset,P.md_stayward_consult,P.criteria_met,P.md_stayward_response,P.psa,P.created_date,'
                . 'P.care_unit_id,CI.name as care_unit_name,P.doctor_id,DOC.name as doctor_name,P.culture_source,P.organism,P.precautions,CS.name as culture_source_name,Org.name as organism_name,Pre.name as precautions_name,P.md_steward_id,U.first_name as md_steward,'
                . 'PC.initial_rx,IRX.name as initial_rx_name,PC.initial_dx,IDX.name as initial_dx_name,PC.initial_dot,'
                . 'PC.new_initial_rx,IRX2.name as new_initial_rx_name,PC.new_initial_dx,IDX2.name as new_initial_dx_name,PC.new_initial_dot,PC.comment,PC.additional_comment_option',
                'join' => array(array('care_unit CI', 'CI.id=P.care_unit_id', 'inner'),
                    array('doctors DOC', 'DOC.id=P.doctor_id', 'inner'),
                    array('users U', 'U.id=P.md_steward_id', 'inner'),
                    array('patient_consult PC', 'PC.patient_id=P.id', 'inner'),
            ...
            if ($emailIs == true) {
                ...
                if ($patient_list) {
                    $return['status'] = 1;
                    $filteredData = $this->applyAlgo($patient_list);
                    $return['response'] = $filteredData;
                    $return['message'] = 'Successfully records found11';
                } else {
                    $return['status'] = 0;
                    $return['message'] = 'Records not found';
                }
    ...
    public function applyAlgo($results_row)
    {
        ...
        // die('jjjj');
// print_r($results_row->md_steward_response); die;
    ...
    function update_patient_post() {
        ...
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('patient_id', 'Patient Id', 'trim|required');
        $this->form_validation->set_rules('md_stayward_response', 'MD Steward Response', 'trim|required|in_list[Agree,Disagree,NoResponse,Modify]');
        $this->form_validation->set_rules('additional_comment_option', 'Additional_comment_option', 'trim');
        $this->form_validation->set_rules('psa', 'Provider Steward Agreement', 'trim');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($data['md_stayward_response']) && !empty($data['md_stayward_response'])) {
                $option = array(
                    ...
                    'where' => array('id' => $data['patient_id'])
                );
                /* if(!empty($this->input->post('pct'))){
                    $option['data']['pct'] = $this->input->post('pct');
                } */
            ...
            $option = array(
                'table' => 'patient',
                'select' => 'id,md_steward_id,operator_id,care_unit_id,patient_id',
                'single' => true
            );
            $option['where']['id'] = $data['patient_id'];
            $patient_list = $this->common_model->customGet($option);
            ...
            $option = array(
                ...
                'data' => array(
                    ...
                    'additional_comment_option' => (!empty($this->input->post('additional_comment_option'))) ? $this->input->post('additional_comment_option') : null,
    ...
    function update_patients_complete_details_post() {
        $data = $this->input->post();
       // print_r($data);die;
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('patient_id', 'Patient Id', 'trim|required');
       // $this->form_validation->set_rules('md_stayward_response', 'MD Steward Response', 'trim|required|in_list[Agree,Disagree,NoResponse]');
        //$this->form_validation->set_rules('additional_comment_option', 'Additional_comment_option', 'trim');
       // $this->form_validation->set_rules('psa', 'Provider Steward Agreement', 'trim');
        $this->form_validation->set_rules('care_unit_id', 'Care Unit', 'trim|required');
        $this->form_validation->set_rules('doctor_id', 'Provider Doctor', 'trim|required');
      ...
      //  $this->form_validation->set_rules('new_initial_dot', 'New Initial DOT', 'trim');
        ...
        if ($this->form_validation->run() == FALSE) {
           ...
           if('infection_surveillance_checklist' != 'N/A'){
                $x = $this->input->post('criteria_met');
           ...
           // if (isset($data['md_stayward_response']) && !empty($data['md_stayward_response'])) {
                $option = array(
                    ...
                    'data' => array(
                        ...
                        'care_unit_id' => $this->input->post('care_unit_id'),
                        'doctor_id' => $this->input->post('doctor_id'),
                        'md_steward_id' => $this->input->post('md_steward_id'),
                        'symptom_onset' => $this->input->post('symptom_onset'),
                        'culture_source' => $this->input->post('culture_source'),
                        'organism' => $this->input->post('organism'),
                        'precautions' => $this->input->post('precautions'),
            ...
            $option = array(
                ...
                'data' => array(
                   ...
                   // 'additional_comment_option' => (!empty($this->input->post('additional_comment_option'))) ? $this->input->post('additional_comment_option') : null,
                    ...
                    'initial_dot' => (!empty($this->input->post('initial_dot'))) ? $this->input->post('initial_dot') : 0,
     ...
     * Description:   To Get All category
     */
    function update_patient_new_post() {
        $data = $this->input->post();
        $return['code'] = 200;
        $this->form_validation->set_rules('login_session_key', 'Login session key', 'trim|required|callback__validate_login_session_key');
        $this->form_validation->set_rules('patient_id', 'Patient Id', 'trim|required');
        $this->form_validation->set_rules('md_stayward_response', 'MD Stayward Response', 'trim|in_list[Agree,Disagree,NoResponse,Modify]');
        $this->form_validation->set_rules('additional_comment_option', 'Additional_comment_option', 'trim');
        $this->form_validation->set_rules('new_initial_dot', 'New Initial Days of Therapy', 'trim');
        $this->form_validation->set_rules('comment', 'Comment', 'trim');
        $this->form_validation->set_rules('infection_surveillance_checklist', 'Infection Surveillance Checklist', 'trim');
    ...
    function send_mail_smtp1_post($template2,$to_email)
    {
    ...
    function email_smtp1_post()
    {
        $data = $this->input->post();
        //$this->form_validation->set_rules('email', 'Email Id', 'trim|required|valid_email');

       $patient_information_for_mail= $this->patient_details_post();

        $to_email= $_POST['email'];

        $patient_id1 = $patient_information_for_mail->patient_id;
        $care_unit_name1 = ucwords($patient_information_for_mail->care_unit_name);
        $doctor_name1 = ucwords($patient_information_for_mail->doctor_name);
        if($patient_information_for_mail->symptom_onset == 'Facility'){
        $symptom_onset1 = 'Facility/HAI';
        }
        else if($patient_information_for_mail->symptom_onset == 'Hospital'){
            $symptom_onset1 = 'Hospital/CAI';
        }else{
            $symptom_onset1 = '';
        }
        $culture_source1 = $patient_information_for_mail->culture_source_name;
        $organism1 = $patient_information_for_mail->organism_name;
        $precautions = $patient_information_for_mail->precautions_name;
        $md_steward1 = ucwords($patient_information_for_mail->md_steward);
        $date_of_start_abx1 = date('m/d/Y',strtotime($patient_information_for_mail->date_of_start_abx));
        $initial_rx_name1 = $patient_information_for_mail->initial_rx_name;
        $initial_dx_name1 = $patient_information_for_mail->initial_dx_name;
        $initial_dot1 = $patient_information_for_mail->initial_dot;
        $infection_surveillance_checklist1 = $patient_information_for_mail->infection_surveillance_checklist;
        $criteria_met1 = $patient_information_for_mail->criteria_met;
        $md_stayward_response1 = $patient_information_for_mail->md_stayward_response;
        $new_initial_rx_name1 = $patient_information_for_mail->new_initial_rx_name;
        $psa1 = $patient_information_for_mail->psa;
        $new_initial_dx_name1 = $patient_information_for_mail->new_initial_dx_name;
        $new_initial_dot1 = $patient_information_for_mail->new_initial_dot;
        //$pct1 = $patient_information_for_mail->pct;
        $additional_comment_option1 = str_replace( array('[','"',']') , ''  ,$patient_information_for_mail->additional_comment_option);
       ...
       <table border="1" width="100%" cellpadding="5">
        <tr>
         ...
         <td width="50%">'. $patient_id1 .'</td>
        </tr>
        <tr>
         <td width="50%">Care Unit</td>
         <td width="50%">'.$care_unit_name1.'</td>
        </tr>
        <tr>
         <td width="50%">Provider MD</td>
         <td width="50%">'.$doctor_name1.'</td>
        </tr>
        <tr>
         <td width="50%">Infection Onset</td>
         <td width="50%">'.$symptom_onset1.'</td>
        </tr>
        <tr>
         <td width="50%">MD Steward</td>
         <td width="50%">'. $md_steward1 .'</td>
        </tr>
        <tr>
         <td width="50%">Date of start abx</td>
         <td width="50%">'.$date_of_start_abx1.'</td>
         </tr>
       </table>
       </br>
       ...
       <table border="1" width="100%" cellpadding="5">
        <tr>
         <td width="50%">Antibiotic Name</td>
         <td width="50%">'. $initial_rx_name1 .'</td>
        </tr>
        <tr>
         <td width="50%">Diagnosis</td>
         <td width="50%">'. $initial_dx_name1 .'</td>
        </tr>
        <tr>
         <td width="50%">Initial Days of Therapy</td>
         <td width="50%">'. $initial_dot1 .'</td>
        </tr>
        <tr>
         <td width="50%">ABX Checklist</td>
         <td width="50%">'. $infection_surveillance_checklist1 .'</td>
        </tr>
        <tr>
         <td width="50%">Criteria Met</td>
         <td width="50%">'.$criteria_met1.'</td>
        </tr>
        <tr>
         <td width="50%">Culture Source</td>
         <td width="50%">'.$culture_source1.'</td>
        </tr>
        <tr>
         <td width="50%">Organism</td>
         <td width="50%">'.$organism1.'</td>
        </tr>
        <tr>
         <td width="50%">Precautions</td>
         <td width="50%">'.$precautions.'</td>
        </tr>
       </table>
       </br>
       ...
       <table border="1" width="100%" cellpadding="5">
        <tr>
         <td width="50%">MD Steward Response</td>
         <td width="50%">'.$md_stayward_response1.'</td>
        </tr>
        <tr>
         <td width="50%">NEW Antibiotic Name</td>
         <td width="50%">'.$new_initial_rx_name1.'</td>
        </tr>
        <tr>
         <td width="50%">PSA(Provider Steward Agreement)</td>
         <td width="50%">'. $psa1.'</td>
        </tr>
        <tr>
         <td width="50%">New Diagnosis</td>
         <td width="50%">'. $new_initial_dx_name1 .'</td>
        </tr>
        <tr>
         <td width="50%">New Days of Therapy</td>
         <td width="50%">'.$new_initial_dot1.'</td>
        </tr>
       <tr>
        <td width="50%">Additional Comment</td>
        <td width="50%">'.$additional_comment_option1.'</td>
       </tr>
       </table>';


        $template2 = $this->send_mail_smtp1_post($template3,$to_email);
       
        return $template2;
    }
    

public function search_name_for_email_post(){
    ...
    $data = array();
    $name = isset($_REQUEST['first_name']) ? $_REQUEST['first_name'] : '';

    if ($name != null) {
        $this->db->like('last_name',$name);
        $this->db->select('first_name, last_name, email,id');
        $result = $this->db->get("vendor_sale_users");
    }
    else {
        $result = $this->db->get("vendor_sale_users");
    }
