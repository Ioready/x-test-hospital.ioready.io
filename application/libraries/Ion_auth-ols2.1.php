...
class Ion_auth {
    ...
    public function forgotten_password_app($identity) {    //changed $email to $identity
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            // Get user information
            $identifier = $this->ion_auth_model->identity_column; // use model identity column, so it can be overridden in a controller
            $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
            ...
            if (!empty($user)) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_forgot_password_app', 'ion_auth'), $data, true);
                    $this->email->clear();
                    $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                    $this->email->to($user->email);
                    $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
    ...
    public function forgotten_password_web($identity) {    //changed $email to $identity
        if ($this->ion_auth_model->forgotten_password($identity)) {   //changed
            // Get user information
            $identifier = $this->ion_auth_model->identity_column; // use model identity column, so it can be overridden in a controller
            $user = $this->where($identifier, $identity)->where('active', 1)->users()->row();  //changed to get_user_by_identity from email
            ...
            if (!empty($user)) {
                ...
                if (!$this->config->item('use_ci_email', 'ion_auth')) {
                    ...
                    $message = $this->load->view($this->config->item('email_templates', 'ion_auth') . $this->config->item('email_forgot_password_web', 'ion_auth'), $data, true);
                    $this->email->clear();
                    $this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
                    $this->email->to($user->email);
                    $this->email->subject($this->config->item('site_title', 'ion_auth') . ' - ' . $this->lang->line('email_forgotten_password_subject'));
    ...
    public function is_admin($id = false) {
        ...
        return $this->in_group('admin', $id);
    }

    public function is_vendor($id = false) {
        $this->ion_auth_model->trigger_events('is_vendor');

        return $this->in_group('Vendor', $id);
    }

     public function is_subAdmin($id = false) {
        $this->ion_auth_model->trigger_events('is_subAdmin');

        return $this->in_group('SubAdmin', $id);
    }

    public function is_facilityManager($id = false) {
        $this->ion_auth_model->trigger_events('is_facilityManager');

        return $this->in_group('FacilityManager', $id);
    }

    public function is_asset($id = false) {
        $this->ion_auth_model->trigger_events('is_asset');

        return $this->in_group('asset_owner', $id);
    }

    public function is_patient($id = false) {
        $this->ion_auth_model->trigger_events('is_patient');
